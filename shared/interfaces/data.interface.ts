...
import { Link, Node, NodeLink } from '../../src/features/dwdm-circuit/shapes';
...
export function load(graph: dia.Graph, data: IData) {
  ...
  data.nodes.forEach((node) => {
    ...
    cards.forEach(
      ({ id: childId, ctor, x: childX, y: childY, ports, links }) => {
        const cardModel = new ctor({
          id: childId,
          position: { x: x + childX, y: y + childY + Node.HEADER_HEIGHT },
          ports: {
            items: ports.map(({ id, group }) => ctor.createPort(id, group)),
          },
        ...
        Object.keys(links).forEach((sourcePortId) => {
          links[sourcePortId].forEach(({ ctor, id, port }) => {
            ...
            let link =
              linksMap[`${id}:${port}:${cardModel.id}:${sourcePortId}`];
            if (link) {
